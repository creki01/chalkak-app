...
// 🔥 가사 추출 로직 전면 개편 (환각 방지 및 원어 번역 후 검색 도입)
app.post('/api/fetch-lyrics', async (req, res) => {
    try {
        ...
        // 1단계: Gemini에게 한국어가 섞인 제목을 정확한 '원어 제목'과 '원어 아티스트명'으로 변환하도록 강제
...
2. 당신이 이 곡의 **공식 원어 가사(Official Original Lyrics)**를 완벽하게 알고 있다면 "lyrics"에 작성하세요.
3. **[강력 경고]** 가사를 정확히 모르거나 확신할 수 없다면, **절대로 가짜 가사를 지어내거나 창작하지 말고** "lyrics" 값을 반드시 빈칸("")으로 두세요! 인공지능이 임의로 가사를 작사하는 환각(Hallucination) 현상을 엄격히 금지합니다.
4. 가사 텍스트 안에 [Verse], [Chorus] 등의 파트 태그나 곡 설명문은 넣지 마세요. 오직 가사 원문만 작성하세요.

반드시 아래 JSON 형식으로만 응답하세요. 다른 부연 설명은 생략합니다.
{
  "originalArtist": "원어 아티스트명 (예: 松田聖子)",
  "originalTitle": "원어 곡 제목 (예: 青い珊瑚礁)",
  "lyrics": "정확한 원어 공식 가사 (모르면 반드시 빈칸)"
}`;
        ...
        // 2단계: AI가 100% 확신하는 가사 데이터가 있다면 바로 응답 반환
        if (songInfo.lyrics && songInfo.lyrics.trim().length > 20) {
            const cleanLyrics = songInfo.lyrics.replace(/\[.*?\]/g, '').replace(/^\s*[\r\n]/gm, '\n').trim();
            return res.json({ lyrics: cleanLyrics, source: 'gemini (identified)' });
        }

        // 3단계: AI가 가사를 모른다고 빈칸을 줬다면, 알아낸 '원어 제목(예: 青い珊瑚礁)'으로 해외 공식 가사 DB 검색!
        const searchArtist = songInfo.originalArtist || channelTitle;
        const searchTitle = songInfo.originalTitle || videoTitle;

        try {
            // lrclib 글로벌 검색 (이제 정확한 일본어가 들어가므로 검색 성공률이 크게 오릅니다)
