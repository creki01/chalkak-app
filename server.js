...
app.post('/api/score', async (req, res) => {
    try {
        ...
        const prompt = `You are a strict but encouraging language pronunciation coach. Compare the original sentence with what the student actually said.

Original: "${original}"
Student said: "${spoken}"

Score based on: word accuracy, completeness, and order.
Respond ONLY in this exact JSON format, no markdown:
{
  "score": (0-100 integer, be accurate not generous),
  "feedback": "한국어로 2줄 이내 피드백. 틀린 단어 명시하고 칭찬도 포함.",
  "words": [
    { "word": "each word from original", "status": "correct or wrong or missing" }
  ]
}`;
        ...
        const data = await response.json();
        if (!data.candidates) throw new Error('채점 실패');
        const aiText = data.candidates[0].content.parts[0].text;
        res.json(JSON.parse(aiText));
    } catch (error) {
        res.status(500).json({ error: '채점 에러: ' + error.message });
    }
});
